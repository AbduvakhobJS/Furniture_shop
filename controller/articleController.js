...
exports.getOne = async (req, res, next) => {
    const result = new NewClass(ArticleModel, req, res, next)
    result.getOne()
}
...
exports.updateFile = async (req, res, next) => {
    ...
    const trashFile = await ArticleModel.findById(id)
    const trashImage = trashFile.image 
    for(let item of trashImage) {
        const trashPath = path.join(__dirname, `../public/articleImage/${item}`)
    ...
    const updateFile = await ArticleModel.findByIdAndUpdate(id)
