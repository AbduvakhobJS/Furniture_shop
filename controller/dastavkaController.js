const DastavkaModel = require('../model/dastavkaModel')
...
exports.getOne = async (req, res, next) => {
    const result = new NewClass(DastavkaModel, req, res, next)
    result.getOne()
}
...
exports.updateFile = async (req, res, next) => {
    ...
    const trashFile = await DastavkaModel.findById(id)
    const trashImage = trashFile.image 
    for(let item of trashImage) {
        const trashPath = path.join(__dirname, `../public/dastavkaImage/${item}`)
    ...
    const updateFile = await DastavkaModel.findByIdAndUpdate(id)
...
exports.deleteFile = async (req, res, next) => {
    ...
    await DastavkaModel.findByIdAndDelete(id)
    res.json({
        message: "🤣🤣🤣🤣"
    })
